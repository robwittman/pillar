...
func (r *TriggerRepository) Create(ctx context.Context, trigger *domain.Trigger) error {
	...
	err = r.pool.QueryRow(ctx,
		`INSERT INTO triggers (id, source_id, agent_id, name, filter, task_template, enabled)
...
func (r *TriggerRepository) Get(ctx context.Context, id string) (*domain.Trigger, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT id, source_id, agent_id, name, filter, task_template, enabled, created_at, updated_at
		 FROM triggers WHERE id = $1`, id,
	).Scan(&t.ID, &t.SourceID, &t.AgentID, &t.Name, &filterJSON, &t.TaskTemplate, &t.Enabled, &t.CreatedAt, &t.UpdatedAt)
	if err != nil {
...
func (r *TriggerRepository) ListBySource(ctx context.Context, sourceID string) ([]*domain.Trigger, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, source_id, agent_id, name, filter, task_template, enabled, created_at, updated_at
		 FROM triggers WHERE source_id = $1 AND enabled = true ORDER BY created_at ASC`, sourceID)
	if err != nil {
...
func (r *TriggerRepository) Update(ctx context.Context, trigger *domain.Trigger) error {
	...
	tag, err := r.pool.Exec(ctx,
		`UPDATE triggers SET source_id = $2, agent_id = $3, name = $4, filter = $5, task_template = $6, enabled = $7
		 WHERE id = $1`,
		trigger.ID, trigger.SourceID, trigger.AgentID, trigger.Name,
		filterJSON, trigger.TaskTemplate, trigger.Enabled,
	)
